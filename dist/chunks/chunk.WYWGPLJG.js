...
(function(w) {
    ...
    function b(b2, a2) {
        ...
        for (m = 0; m < f.length; m += 1) {
          ...
          if (r[p2][q] == null)
            for (var n2 = -2; 2 >= n2; n2 += 1)
              for (var l = -2; 2 >= l; l += 1)
                r[p2 + n2][q + l] = n2 == -2 || n2 == 2 || l == -2 || l == 2 || n2 == 0 && l == 0;
        }
      for (f = 8; f < d - 8; f += 1)
      ...
      if (g == null) {
        ...
        for (n2 = 0; n2 < b2.length; n2 += 1) {
          ...
          m = Math.max(m, w2);
          p2[n2] = Array(v2);
          for (l = 0; l < p2[n2].length; l += 1)
            p2[n2][l] = 255 & f.B()[l + u2];
          u2 += v2;
          l = y.C(w2);
        ...
        for (l = u2 = 0; l < k; l += 1)
          for (n2 = 0; n2 < b2.length; n2 += 1)
            l < p2[n2].length && (f[u2] = p2[n2][l], u2 += 1);
        for (l = 0; l < m; l += 1)
          for (n2 = 0; n2 < b2.length; n2 += 1)
      ...
      for (q = d - 1; 0 < q; q -= 2)
        for (q == 6 && --q; ; ) {
          for (n2 = 0; 2 > n2; n2 += 1)
            r[k][q - n2] == null && (l = false, p2 < b2.length && (l = (b2[p2] >>> m & 1) == 1), a2(k, q - n2) && (l = !l), r[k][q - n2] = l, --m, m == -1 && (p2 += 1, m = 7));
          k += f;
          if (0 > k || d <= k) {
            k -= f;
            f = -f;
  ...
  var A = { L: 1, M: 0, Q: 3, H: 2 }, y = function() {
    ...
    var a = [
      ...
      for (var a2 = b.h(), c2 = 0, d = 0; d < a2; d += 1)
        for (var g = 0; g < a2; g += 1) {
          for (var e3 = 0, t2 = b.a(d, g), p2 = -1; 1 >= p2; p2 += 1)
            if (!(0 > d + p2 || a2 <= d + p2))
              for (var q = -1; 1 >= q; q += 1)
                0 > g + q || a2 <= g + q || (p2 != 0 || q != 0) && t2 == b.a(d + p2, g + q) && (e3 += 1);
          5 < e3 && (c2 += 3 + e3 - 5);
        }
      for (d = 0; d < a2 - 1; d += 1)
        for (g = 0; g < a2 - 1; g += 1)
    ...
    var a = [
      ...
      a2 = [];
      for (var d = 0; d < b; d += 1)
        for (var g = e3[3 * d], h = e3[3 * d + 1], t2 = e3[3 * d + 2], p2 = 0; p2 < g; p2 += 1) {
          var q = t2, f = {};
          f.o = h;
          f.j = q;
          a2.push(f);
